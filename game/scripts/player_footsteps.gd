...
@export var footstep_delay_running: float = 0.4
...
var last_footstep_sound_index: int = -1
#var current_footstep_sound_index: int = -1
...
func play_footstep_sound():
	...
	if stands_on_leafs() or jumped_from_leafs():
		...
		sound_index = randi() % footstep_sounds_on_leafs.size()
		while sound_index == last_footstep_sound_index:
			sound_index = randi() % footstep_sounds_on_leafs.size()
		last_footstep_sound_index = sound_index
		
		# play the footsteps sounds in order
		#current_footstep_sound_index += 1
		#if current_footstep_sound_index >= footstep_sounds_on_leafs.size():
		#	current_footstep_sound_index = 0
		#sound_index = current_footstep_sound_index
		
		# set and play it
		var sound = footstep_sounds_on_leafs[sound_index]
